...
public class BusqAscABB {
	...
	static public void main(String[] agrs){
	   ...
	   for(int i=0;i<=1000;i++){
	        abb.insertar(i); // insercion de elementos dentro del arbol
	   }
	       
	   long start = System.currentTimeMillis();
	   for(int i=0;i<=100;i++){//se buscan 100 elementos aleatorios dentro del arbol
	        abb.buscar(resultado1[i]);
	   }
	   long finish = System.currentTimeMillis() - start;
	   System.out.println("La busqueda para un ABB con llaves ascendentes demora "+finish+" ms");
	}
}
